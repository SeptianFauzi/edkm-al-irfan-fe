...
import { getpesertaZakatFitrahSent, getpesertaZakatFitrahSentSent, getZakatFitrahList, pesertaZakatFitrahSent, pesertaZakatFitrahSentSent } from '../../../config/redux/action'
import * as yup from "yup";
import { yupResolver } from '@hookform/resolvers/yup'
import { useForm } from 'react-hook-form'
import TableTambahZakatFitrahSent from '../../../components/moleculs/TableTambahZakatFitrahSent'
import { GetPesertaZakatFitrahSentList } from '../../../config/redux/services'
import { SearchRounded, WarningRounded } from '@material-ui/icons'
import TableZakatFitrahSent from '../../../components/moleculs/TableZakatFitrahSent'
function AddZakatFitrahSent() {
    ...
    useEffect(() => {
        dispatch(GetPesertaZakatFitrahSentList(yearhijriah));
    }, []);
                    ...
                    <Box my={2}>
                        <Breadcrumbs aria-label="breadcrumb">
                            ...
                            <NavLink color="secondary" to={"/admin/penerimazakatfitrah/tambah"}>
                                Tambah Peserta Penerima Zakat Fitrah
                            </NavLink>
                        </Breadcrumbs>
                    </Box>
                </Grid>
                <Card variant="outlined" style={{ minHeight: '80vh' }}>
                    ...
                    <CardContent>
                        <Grid container justify="center" direction="row">
                            <Typography variant="h5" align="center">Tambah Peserta Penerima Zakat Fitrah</Typography>
                        </Grid>
                        <Hidden smDown>
                            <Grid direction="column" container justify="flex-end" alignItems="flex-end" style={{ marginBlock: '10px' }}>
                                <Grid direction="row" container justify="flex-end" alignContent="flex-end" spacing={2}>
                                    ...
                                    <Grid item >
                                        <Button style={{ paddingBlock: '1.1em' }} variant="contained" color="primary" disabled={state.loading || errors.tahunhijriah} onClick={() => dispatch(GetPesertaZakatFitrahSentList(yearhijriah))}><SearchRounded></SearchRounded></Button>
                                    </Grid>
                                </Grid>
                            </Grid>
                            <Grid item xs={12}>
                                {!state.loading && state.pesertaZakatFitrahSent ? <TableTambahZakatFitrahSent dataRow={state.pesertaZakatFitrahSent} yearHijriah={yearhijriah} /> : <TableTambahZakatFitrahSent dataRow={[]} yearHijriah={yearhijriah} />}
                            </Grid>
                        </Hidden>
                        <Hidden mdUp>
                            <Grid container
...
export default AddZakatFitrahSent
