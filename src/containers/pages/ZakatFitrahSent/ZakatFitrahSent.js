...
import { GetZakatFitrahSentList } from '../../../config/redux/services';
function ZakatFitrahSent(props) {
    ...
    useEffect(() => {
        dispatch(GetZakatFitrahSentList(yearhijriah));
    }, []);
                    ...
                    <Grid item xs={12}>
                        <Box my={2}>
                            <Breadcrumbs aria-label="breadcrumb">
                                <NavLink color="inherit" to={"/admin/penerimazakatfitrah"}>
                        ...
                        <CardContent>
                            <Hidden mdDown>
                                <Grid container direction="row" justify="center" alignItems="center">
                                    <Grid item xs={12}>
                                        <Box>
                                            <Typography variant="h5" align="center" mb={20}>Daftar Peserta Penerima Zakat Fitrah</Typography>
                                        </Box>
                                    </Grid>
                                    <Grid item xs={12}>
                                        <Grid container direction="row" justify="space-between" alignItems="flex-start">
                                            <Grid item>
                                                <Button color="primary" style={{ paddingBlock: '1.1em' }} variant="contained" disabled={state.loading || state.loadingData}><NavLink to="/admin/penerimazakatfitrah/tambah" style={{ color: 'white' }}><Add /></NavLink></Button>
                                            </Grid>
                                            <Grid item>
                                                <Grid direction="column" container justify="flex-end" alignItems="flex-end" style={{ marginBlock: '10px' }}>
                                                    <Grid direction="row" container justify="flex-end" alignContent="flex-start" spacing={1}>
                                                        ...
                                                        <Grid item>
                                                            <Button style={{ paddingBlock: '1.1em' }} variant="contained" color="primary" disabled={state.loading || errors.tahunhijriah} onClick={() => dispatch(GetZakatFitrahSentList(yearhijriah))}><SearchRounded></SearchRounded></Button>
                                                        </Grid>
                                                    </Grid>
                                ...
                                {!state.loading && state.zakatFitrahSent ? <TableZakatFitrahSent dataRow={state.zakatFitrahSent} /> : <TableZakatFitrahSent dataRow={[]} />}
                            </Hidden>
                            <Hidden lgUp>
                                <Grid container
                                    direction="column"
...
export default ZakatFitrahSent
